...
class Scene15(RNGDataFlow):
    ...
    def __init__(self, dir, name, img_size, meta_dir=None,
        ...
        imgs_mean = np.mean(self.imgs, axis=(0,1), keepdims=True)
        self.imgs -= imgs_mean
        imgs_std = np.std(self.imgs, axis=(0,1), keepdims=True)
        self.imgs /= imgs_std
        ########################################################
...
def get_data(datadir, task, train_or_test):
    ...
    if isTrain:
        augmentors = [
            ...
            imgaug.GaussianNoise(sigma=.03),
            #imgaug.Contrast((.95,1.05)),
            #imgaug.BrightnessScale((.95,1.05)), 
            imgaug.Affine(scale=(.98,1.02), translate_frac=(.2,0)) 
        ]
    else:
