...
def get_tiny_images(image_paths):
    ...
    resized_images = np.zeros((images_n, 256))
    for i, image_path in enumerate(image_paths):  
        loaded_image = imread(image_path)
        original_height = loaded_image.shape[0]
        original_width = loaded_image.shape[1]
        delta_height_width = original_height - original_width
        d_rows = 0 if delta_height_width < 0 else -1*delta_height_width
        d_columns = 0 if delta_height_width > 0 else delta_height_width
        square_image = loaded_image[0:original_height + d_rows, 0:original_width + d_columns]
        resized_images[i] = resize(rgb2grey(square_image), (16, 16), anti_aliasing=True).reshape(256)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    #TODO: Implement this function!
    images_features = np.zeros((1,36))
    for path in image_paths:
        image = imread(path)
        hog_feature = hog(image, pixels_per_cell=(16, 16), cells_per_block=(2, 2), block_norm='L1', feature_vector=True)
        image_features = hog_feature.reshape(-1, 36) #len(image_features¸) = 64.... i think... but wrong
        images_features = np.append(images_features, image_features, axis=0)
    images_features = images_features[1:]
    k_means = KMeans(n_clusters=vocab_size).fit(images_features)
...
def get_bags_of_words(image_paths):
    ...
    n = len(image_paths)
    d = len(vocab)
    image_histograms = np.zeros((n, d))
    for i, path in enumerate(image_paths):
        image = imread(path)
        hog_feature = hog(image, pixels_per_cell=(16,16), cells_per_block=(2, 2), block_norm='L1', feature_vector=True)
        #print('[!!]', hog_feature.shape)
        image_features = hog_feature.reshape(-1, 36)
        #print('[!!!]', image_features.shape)
        n_distances = cdist(image_features, vocab, metric='cosine')
        for distances in n_distances:
            #print('[!!]', distances)
            max_i = np.argsort(distances)[0]
            #print('[!]', max_i)
            image_histograms[i, max_i] += 1
    for i, image_histogram in enumerate(image_histograms):
        image_histograms[i] = image_histogram/np.linalg.norm(image_histogram)
    # print('[!]', image_histograms)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # e.g., cdist
    distances = cdist(test_image_feats, train_image_feats, 'cosine')

    #TODO:
    # 1) Find the k closest features to each test image feature in euclidean space
    nk_closest_features = np.zeros((n, k))
    for i, distance in enumerate(distances):
        indices = np.argpartition(distance, k)[:k]
        nk_closest_features[i] = indices
    # 2) Determine the labels of those k features
    # 3) Pick the most common label from the k
    # 4) Store that label in a list
    predicted_labels = list()   
    # k_closest_features tracks indices 
    for k_closest_features in nk_closest_features:
        labels = list()
        for i in k_closest_features:
            labels.append(train_labels[int(i)])
        c = Counter(labels)
